...
# Load the XML file
tree = etree.parse("../SloKJV_sword.xml")
root = tree.getroot()
...
# Write JS to a separate file (unchanged)
js_content = """
...
# Get all books and their short titles
...
# Generate book navigation HTML
def generate_book_nav(main_title):
    nav_html = [
        f'    <h1><a href="index.html">{main_title}</a></h1>',  # Spremenjeno iz index.htm v index.html
        '    <div class="book-nav">',
        '      <h3>Stara zaveza</h3>',
...
main_title = root.findtext("osis:header/osis:work/osis:title", default="Sveto pismo Kralja Jakoba (1769) – SloKJV", namespaces=ns)
rights = root.findtext("osis:header/osis:work/osis:rights[@type='x-copyright']", default="Licenca: CC BY-NC-ND 4.0", namespaces=ns)
source = root.findtext("osis:header/osis:work/osis:source", default="TextSource=github.com/msavli/SloKJV", namespaces=ns)
...
# Generate index.html with descriptions as a list
descriptions = root.findall("osis:header/osis:work/osis:description", namespaces=ns)
...
index_content = [
    ...
    "    <div>",
    "      <ul>",
] + [f"        <li>{desc.text}</li>" for desc in descriptions if desc.text] + [
    "      </ul>",
    "    </div>",
...
with open(f"{output_dir}/index.html", "w", encoding="utf-8") as f:  # Spremenjeno iz index.htm v index.html
    f.write("\n".join(index_content))
print(f"Wrote file: {output_dir}/index.html")

def process_note_content(note_elem):
    """Obdelaj vsebino znotraj <note>, vključno z <hi type='bold'>, z ustreznimi presledki"""
...
def process_colophon_content(colophon_elem):
    """Obdelaj vsebino znotraj <div type='colophon'>, vključno z <transChange type='added'>"""
...
for i, book in enumerate(books):
    ...
    for chapter in chapters:
        ...
        # Obdelava colophon na ravni poglavja
        ...
        # Dodaj povezavo na vrh strani na koncu poglavja (po colophonu)
        html_content.append('    <div class="top-link"><a href="#">⇧ Na vrh</a></div>')

    # Obdelava colophon na ravni knjige
    ...
    for colophon in colophons:
        ...
        # Dodaj povezavo na vrh strani tudi po colophonu na ravni knjige
        html_content.append('    <div class="top-link"><a href="#">⇧ Na vrh</a></div>')

    # Dodaj navigacijo na prejšnjo/naslednjo knjigo in index
    ...
    nav_links.append('      <a href="index.html">Kazalo</a>')  # Spremenjeno iz index.htm v index.html
    if next_book:
